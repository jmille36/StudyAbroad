...
class PrefPage(Frame):
    ...
    def __init__(self,parent,project):
        ...
        reg_lab.grid(row=2, column=0, padx=5, pady=5)
        reg_option = ["Africa - Central", "Africa - East", "Africa - North", "Africa - South", "Africa - West",
        ...
        reg_en.grid(row=2, column=1, columnspan=5, padx=5, pady=5)

        seclang_lab = Label(self, text="Second Languages")
        seclang_lab.grid(row=3, column=0, padx=5, pady=5)
        seclang_option = ["None","Arabic", "French", "German", "Hindi", "Italian", "Japanese", "Korean", "Mandarin",
        ...
        seclang_en.grid(row=3, column=1, columnspan=5, padx=5, pady=5)

        cost_lab = Label(self, text="Cost range")
        cost_lab.grid(row=4, column=0, padx=5, pady=5)
        cost_en_min = Entry(self)
        cost_en_min.grid(row=4, column=1, columnspan=2, padx=5, pady=5)
        to = Label(self, text="to")
        to.grid(row=4, column=3, columnspan = 2, padx=5, pady=5)
        cost_en_max= Entry(self)
        cost_en_max.grid(row=4, column=5, columnspan=2, padx=5, pady=5)
        ...
        acco_en.grid(row=5, column=1, columnspan=5, padx=5, pady=5)

        trp_lab = Label(self, text="Trips")
        trp_lab.grid(row=6, column=0, padx=5, pady=5)
        trp_che_1 = IntVar()
        trp_chebut_1 = Checkbutton(self, text="City Exploring", variable = trp_che_1)
        trp_chebut_1.grid(row=6, column=1, columnspan=2, padx=5, pady=5)
        trp_che_2 = IntVar()
        trp_chebut_2 = Checkbutton(self, text="Sightseeing", variable = trp_che_2)
        trp_chebut_2.grid(row=6, column=3, columnspan=2, padx=5, pady=5)
        trp_che_3 = IntVar()
        trp_chebut_3 = Checkbutton(self, text="Nightlife", variable = trp_che_3)
        trp_chebut_3.grid(row=6, column=5, columnspan=2, padx=5, pady=5)

        seas_lab = Label(self, text="Program Season")
        seas_lab.grid(row=7, column=0, padx=5, pady=5)
        seas_option = ["Spring", "Summer", "Fall", "Winter"]
        seas_var = StringVar(self)
        seas_var.set("Select...")
        seas_en = OptionMenu(self, seas_var, *seas_option)
        seas_en.grid(row=7, column=1, columnspan=5, padx=5, pady=5)
